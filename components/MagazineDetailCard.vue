<template>
  <div class="blog-post">
    ...
    <img :src="magazine?.cover_image_link"
         :alt="magazine?.title"
         ...
         v-lazy-load>
    <p v-lazy-load="contentWithLazy" />

    <div v-if="magazine?.spot">
      <h4>
        Related Spot
      </h4>
      <OverViewCard class="col-sm-12 col-md-8" :spot="magazine.spot"/>
    </div>
  </div>

</template>
<script>
import {lazy} from 'nuxt-lazy-load'

export default {
  name:'MagazineDetailCard',
  props:['magazine'],
  data(){
    return {
    }
  ...
  computed: {
    contentWithLazy() {
      return `<div ${lazy}>${this.magazine?.description}</div>`
    }
  }
